...
describe('Order Management', () => {
    ...
    describe('Order NotABLE', () => {
        ...
        it('[REVERT] Only receiver can use this function', async () => {
            ...
            const result = ordermanagement.connect(distributor).confirmOrder(orderID, invoice, lotID, sku)

            await expect(result).to.be.revertedWith('Only order\'s receiver can use this function')
        })
        ...
        it('[REVERT] Only sender can use this function', async () => {
            ...
            await expect(result).to.be.revertedWith('Only order\'s sender can use this function')
        })
        ...
        it('[REVERT] Order is not confirmable', async () => {
            ...
            await ordermanagement.connect(manufacturer).confirmOrder(orderID, invoice, lotID, sku)
            const result = ordermanagement.connect(manufacturer).confirmOrder(orderID, invoice, lotID, sku)
        ...
        it('[REVERT] Order is not onholdable', async () => {
            ...
            const result = await ordermanagement.connect(manufacturer).onholdOrder(orderID)
        ...
        it('[REVERT] Only registered users can call this function', async () => {
            const orderID = generateRandomOrderID(distributor.address)
            const amount = generateRandomAmount()
            const result = ordermanagement.connect(retailer).createOrder(orderID, manufacturer.address, amount)

            await expect(result).to.be.revertedWith('Only registered users can call this function.')
        })
    })
})